...
- `expression_population_store.py` owns the SQLite mirror of expression-population records and parent-selection records.
- `expression_bridge_variants.py` owns evaluator-side portfolio bridge diagnostics such as slower rebalance, signal decay, and no-trade-band variants. These diagnostics do not change the primary expression status.

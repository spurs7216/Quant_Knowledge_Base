...
def _ranking_row(result: dict[str, Any]) -> dict[str, Any]:
...
def _status_from_metrics(
    ...
    *,
    ...
    hard_gates = _expression_sample_hard_gates(
        metrics,
        portfolio_coverage,
...
def _expression_sample_hard_gates(
    metrics: dict[str, dict[str, float]],
    portfolio_coverage: dict[str, Any],
    *,
    max_weight: float,
    ...
    search = metrics.get("search_sample", {})
    observed_max_weight = _finite_or_none(search.get("max_weight"))
    observed_net = _finite_or_none(search.get("max_abs_net_exposure"))
    observed_missing = _finite_or_none(search.get("max_missing_held_weight"))
    return {
        ...
        "turnover_aware_score_reported": _finite_or_none(search.get("turnover_aware_score")) is not None,
    }

...
| Expression-only search may miss turnover/cost solution | Fixed daily top/bottom bridge may hide lower-turnover ideas | Runner now writes bridge-variant diagnostics for daily, 5-day rebalance, signal decay, and no-trade-band variants | The primary status still uses the default bridge until a bridge policy is reviewed |
...
The next remote expression episode should:

- not focus on attempt017;
- run the population-aware expression runner;
- return `expression_population.sqlite`, `expression_population_ledger.*`, `expression_success_flags.csv`, and `expression_bridge_variants.csv`;

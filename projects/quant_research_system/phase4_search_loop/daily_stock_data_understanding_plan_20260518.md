...
- missingness-aware signal construction;
...
- Are `DlyPrcVol` and `DlyCap` so skewed that raw-scale child edits should be discouraged?
- How often do low-price or low-liquidity names survive the fixed filters and rolling top-500 universe?
- Does SIC2 have enough daily group breadth for industry-neutral ranking without fragile small-group behavior?
- Do return flags or missing-return fields explain missing-held-weight failures?
- Does the top-500 universe have stable enough daily breadth across regimes to support broad long/short books?
- Which transform primitives should become active prompt rules, and which should remain caveats?
